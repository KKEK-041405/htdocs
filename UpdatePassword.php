...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql =mysqli_query($conn,"SELECT * FROM `mbts`.`login_details` WHERE `username` = '$username'"); 
    $rows = mysqli_fetch_assoc($sql);
    $result = mysqli_query($conn,"UPDATE `mbts`.`login_details` SET `Password`='$password',`is_FirstLogin`='0' WHERE `username` = '$username'");
